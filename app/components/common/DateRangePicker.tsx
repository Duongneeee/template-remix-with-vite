...
interface DateRangePickerProps {
  onApply: (selectedRange: { since: Date; until: Date },alias:string) => void;
  selectActiveDateRange?: number;
}
const DateRangePicker = (props: DateRangePickerProps) => {
  const { onApply, selectActiveDateRange=0 } = props;
  ...
  const [activeDateRange, setActiveDateRange] = useState(ranges[selectActiveDateRange]);
  ...
  function apply() {
    ...
    if (activeDateRange && activeDateRange.period) {
      ...
      onApply({ since, until},activeDateRange.alias);
    }
  }
  function cancel() {
    setPopoverActive(false);
